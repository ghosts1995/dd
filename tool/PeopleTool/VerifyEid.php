...
class VerifyEid
{
     ...
     * @return bool
     */
    public static function is_eid($_eid='')
    {
        $_eid = self::to18Card($_eid);
        if (strlen($_eid) != 18)
        {
            return false;
        }
        $cardBase = substr($_eid, 0, 17);
        if(self::getVerifyNum($cardBase) == strtoupper(substr($_eid, 17, 1)))
        {
            return true;
        }
        return false;
    ...
    public static function to18Card($card)
    {
        ...
        if (array_search(substr($card, 12, 3), array('996', '997', '998', '999')))
        {
